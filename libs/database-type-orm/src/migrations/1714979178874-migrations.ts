...
export class Migrations1714979178874 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`user\` ADD \`invited_by\` bigint UNSIGNED NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`user\` ADD \`none_token\` varchar(100) NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`user\` ADD \`is_super_admin\` tinyint NOT NULL DEFAULT '0'`,
    );
  }
}
