...
export class Migrations1715769000026 implements MigrationInterface {
  name = 'Migrations1715769000026';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`user-image\` (\`id\` bigint UNSIGNED NOT NULL AUTO_INCREMENT, \`user_id\` bigint UNSIGNED NOT NULL, \`url\` varchar(255) NOT NULL, \`image_type\` int UNSIGNED NOT NULL COMMENT '1: profile image, 2: wall image', \`is_avatar\` tinyint NOT NULL COMMENT 'true: is current avatar, false: old avatar' DEFAULT 0, \`deleted_at\` datetime(6) NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
